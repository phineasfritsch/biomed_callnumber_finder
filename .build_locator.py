...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .itin-steps .sd{flex:0 0 auto; min-width:230px; font-size:12px; line-height:1.6; color:var(--ink-soft)}
  .itin-steps .sd b{color:var(--ink)}
  .itin-steps .sat{flex:0 0 auto; min-width:210px; font-size:12px; line-height:1.6; color:var(--ink-soft)}
...
<div class="wrap">
  ...
  <div class="lookup">
    ...
    <input id="q" placeholder="call number, book title, author, ISBN…" autocomplete="off" autofocus
  ...
  <div class="cat" id="cat">
    <div class="cat-head" id="catHead">
      <h2>Find a book in the catalog</h2>
  ...
  function walkList(stops,entry,exit,inDoor,outDoor){
    ...
    steps.forEach((st,i)=>{
      ...
      const move = st.shelves===0 ? 'same aisle'
                 : (i===0 || !whole) ? `head ${st.head>0?'east':'west'} from the ${inDoor.name}`
                 : `${st.shelves} ${st.shelves===1?'shelf':'shelves'} ${st.head>0?'east':'west'}`;
      ...
      h+=`<li><span class="sn" style="background:${orderColor(st.n-1,n)}">${st.n}</span>`
       ...
       + `<span class="sat">shelf <b>${st.index}${face[st.side]||''}</b>, ${st.row} row — on your <b>${st.hand}</b></span>`
       + `<span class="scn">${st.cns.join(' · ')}</span>`
       + `<span class="srng">${st.range.start} → ${st.range.end}</span></li>`;
    });
    if(outDoor){
      h+=`<li class="out"><span class="sn out">↺</span><span class="sd">Back to the `
       + `<b>${outDoor.name}</b> to leave this floor.</span></li>`;
    }
    return h+'</ol>';
  }
