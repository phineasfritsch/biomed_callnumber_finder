...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .route-head h2,.cat-head h2{font-family:var(--disp); font-size:17px; font-weight:600; margin:0; display:flex; align-items:center}
...
<div class="wrap">
  ...
  <div class="cat" id="cat">
    <div class="cat-head" id="catHead">
      <h2>Find a book in the catalog<span class="tag">beta</span></h2>
    ...
    <div class="cat-body" id="catBody" hidden>
      <p class="cat-hint">Search UCLA&rsquo;s catalog by title, author or ISBN. The default answers the question asked at the desk all day &mdash; <b>the newest edition of this book, in this building</b> &mdash; so results are scoped to Biomed and ordered newest-first, and widen to the rest of UCLA only if Biomed has nothing. Copies in the <b>Biomed stacks</b> are resolved to an actual shelf you can walk to. The catalog itself returns hits in alphabetical order with no relevance ranking and no spelling correction, so both are done here: a typo of a word or two still finds the book.</p>
      <div class="cat-scope" role="group" aria-label="Where to search">
        <span class="sect-label">Where:</span>
        <button class="pill active" type="button" data-scope="biomed" aria-pressed="true">Biomed</button>
      ...
      <div class="cat-form">
        <label class="sr-only" for="catQ">Search the catalog</label>
        <input id="catQ" placeholder="harrison's principles of internal medicine" autocomplete="off" enterkeyhint="search" spellcheck="false">
        <button class="btn" type="button" id="catGo">Search</button>
        <button class="btn ghost" type="button" id="catClear">Clear</button>
      </div>
      <div class="cat-opts">
        ...
        <label class="chk"><input type="checkbox" id="catBiomed"> Biomed copies only</label>
      ...
      <details class="cat-adv">
        ...
        <div class="cheat">
          ...
          <div><code>at:</code> <code>in:</code></div><div>Scope, overriding the pills: biomed &middot; stacks &middot; reference &middot; desk &middot; ucla</div>
...
(function(){
  ...
  const head=$('catHead'), toggle=$('catToggle'), body=$('catBody'), input=$('catQ'),
...
/* == catalog-core:start ==
  ...
  function classify(ava){
    ...
    if(lib==='BIOMED'){
      ...
      if(known) return Object.assign({here:true, code:j}, known);
      return {kind:'unknown', here:true, code:j,
    ...
    return {kind:'away', here:false, code:j,
            ...
            note:OFFSITE[j]||('Held at '+(ava.c||lib||'another UCLA library')+' \u2014 not in this building.')};
  }
  ...
  /* ---- scopes ----
     `alma.permanentPhysicalLocation` is the only index that actually filters by where a
     copy sits: every library-shaped index (itemLibrary, holding_Library, rep_Library)
     returns 0 for BIOMED in this tenant, probed. So a scope is a set of $j codes and
     nothing else, which is fine — the same nine codes the routing table already knows. */
  const SCOPE={
    biomed:{codes:['bi','biper','biprwt','bian','birf','birs','bicidperm','bicimm','biherb'],
            label:'Biomed \u2014 any location', short:'Biomed'},
    stacks:{codes:['bi','biper','biprwt','bian'],
            label:'Biomed \u2014 shelves you can walk to', short:'Walkable stacks'},
    ref:   {codes:['birf'], label:'Biomed Reference \u00b7 Floor 4', short:'Reference'},
    desk:  {codes:['birs','bicidperm','bicimm','biherb'],
            label:'Biomed service desks (reserves, media, herbarium)', short:'At the desk'},
    ucla:  {codes:null, label:'Every UCLA library', short:'All of UCLA'}
  };
  const SCOPE_ALIAS={biomed:'biomed', bio:'biomed', here:'biomed', building:'biomed',
  ...
  function parseQuery(s){
    const p={text:'', pos:[], neg:[], local:{}, sort:'', scope:'', notes:[], errors:[]};
    const free=[];
    tokenize(s).forEach(tk=>{
      ...
      if(spec.kind==='scope'){
        const k=SCOPE_ALIAS[v.toLowerCase()];
        if(!k) p.errors.push('Unknown scope \u201c'+v+'\u201d. Try biomed, stacks, reference, desk or ucla.');
  ...
  function buildCQL(parsed,mode,scopeKey,sortKey){
    ...
    if(sc && sc.codes) rest.push(orGroup(sc.codes.map(c=>'alma.permanentPhysicalLocation='+c)));
  ...
  function scoreRecord(rec,query,ctx){
    ...
    s-=26*(extra/(extra+7));
    if(rec.hasBiomed) s+=12;               // a tiebreak, not an override: walkable copies first
...
/* == catalog-core:end == */
  ...
  async function sru(query,start,signal,max){
    ...
    for(let attempt=0;attempt<3;attempt++){
      ...
      const p=new URLSearchParams({version:'1.2', operation:'searchRetrieve', recordSchema:'marcxml',
                                   maximumRecords:String(max||PAGE), startRecord:String(start), query:q});
      try{
        const res=await fetch(SRU+'?'+p.toString(),{signal});
        ...
        return {total, records:recs, sorted:!droppedSort && / sortBy /.test(q)};
      }catch(e){
  ...
  function holdingsHtml(rec){
    const keep=biomedOnly.checked
      ? rec.holdings.filter(h=>(h.ava.b||'').toUpperCase()==='BIOMED')
      : rec.holdings.slice();
    const groups={};
    keep.forEach(h=>{
    ...
    // Group by library; Biomed first, everything else after, alphabetically.
    const order=Object.keys(groups).sort((a,b)=>
      (a==='BIOMED'?-1:b==='BIOMED'?1:0) || (a<b?-1:a>b?1:0));
    let h='';
    order.forEach(lib=>{
      const g=groups[lib];
    ...
    if(!keep.length && !rec.online.length)
      h+='<section class="lib away"><div class="hold"><span class="where">'+
         (biomedOnly.checked?'No Biomed copy on this record \u2014 untick \u201cBiomed copies only\u201d to see where else it is held.'
  ...
  let mode='kw', scope='biomed', ctl=null, seq=0,
      loaded=[], total=0, lastQuery='', rankQuery='', relaxedTo='', usedScope='biomed',
      widenReason='', parsed=null, ctx=null;
  ...
  function draw(){
    ...
    if(!cards.length){
      out.innerHTML='<div class="cat-empty">None of the '+kept.length+' ranked records has a copy left to show. '+
        '<b>Untick &ldquo;Biomed copies only&rdquo;</b> to see where else at UCLA they are held.</div>'+footer(hidden);
      wireCards();
      return;
    }
  ...
  const scopeChain=p=>{
    ...
    if(scope==='ucla') return ['ucla'];
    return [scope,'ucla'];                            // Biomed first, the rest of UCLA only if empty
  };

  async function run(reset){
    const eq=effectiveQuery();
    const p=parseQuery(eq.full);
    showApplied(p,eq.extra);
    const hasQuery=!!(p.text||p.pos.length||p.neg.length);
    if(!hasQuery){
      loaded=[]; total=0; out.innerHTML=''; parsed=p;
      setStatus(p.errors.length?esc(p.errors.join(' ')):'Type a title, author or ISBN \u2014 or a filter such as <code>mesh:cardiology year:2015+</code>.',!!p.errors.length);
      input.focus(); return;
    }
    const mine=++seq;
    if(ctl) ctl.abort();
    ...
    const autoMode=detectMode(p.text,mode)||mode;
    ...
    try{
      if(!reset){
        ...
        loaded=[]; total=0; parsed=p; rankQuery=p.text||eq.typed; relaxedTo=''; ctx=null; widenReason='';
        const chain=scopeChain(p);
        const wide=chain[chain.length-1];
        let page=null;
        // Pass 1: the query as typed, narrowest scope first.
        for(let i=0;i<chain.length;i++){
          setStatus('Searching '+esc(SCOPE[chain[i]].label)+'\u2026');
          lastQuery=buildCQL(p,field,chain[i],sortMode);
          const r=await fetchPool(lastQuery,signal,mine);
          if(r===null) return;
          usedScope=chain[i];
          if(r.total){ page=r; if(i>0) widenReason='empty'; break; }
          page=r;
        }
        /* A scoped search that returns *something* has not necessarily returned the book.
           "atlas shrugged" matches exactly one Biomed record \u2014 on a stray keyword, not on
           the title \u2014 and that single hit would otherwise stop the widening dead and answer
           a question nobody asked. So when the best thing in scope does not resemble the
           title at all, spend one cheap page on the wider scope and take it only if it is
           materially better. */
        if(page && page.total && p.text && usedScope!==wide){
          const here=bestScore(page.records,p.text,null);
          if(here<WEAK){
            setStatus('Nothing in '+esc(SCOPE[usedScope].short)+' looks like \u201c'+esc(p.text)+'\u201d \u2014 checking the rest of UCLA\u2026');
            const pr=await sru(buildCQL(p,field,wide,sortMode),1,signal,PROBE);
            if(mine!==seq) return;
            if(pr.total && bestScore(pr.records,p.text,null)>=here+WIDEN_MARGIN){
              lastQuery=buildCQL(p,field,wide,sortMode);
              const full=await fetchPool(lastQuery,signal,mine);
              if(full===null) return;
              if(full.total){ page=full; usedScope=wide; widenReason='weak'; }
            }
          }
        }
        /* Pass 2: nothing matched as typed anywhere. The endpoint has no fuzzy search and no
           "did you mean", so drop one word at a time and let the local scorer fuzzy-match
           what comes back against the original spelling: "wtlas shrugged" -> search
           "shrugged" -> Atlas Shrugged scores top because "wtlas" is one edit from "atlas". */
        if(page && !page.total && p.text && autoMode!=='isbn'){
          /* Try several relaxations with one cheap page each and keep the *best* one, judged
             by how well its top hit scores against what was originally typed. Taking the first
             retry that returns anything is worse than useless: "harrisons principals of
             internal medicin" would settle for a single unrelated record. */
          ...
          for(let i=0;i<alts.length;i++){
            ...
            if(s>=GOOD_ENOUGH) break;     // clearly the right book; stop spending requests
          }
          if(best){
            relaxedTo=best.alt;
            const relaxed=Object.assign({},p,{text:best.alt});
            // Narrow it back down: a misspelt title whose book *is* at Biomed should still
            // come back as a Biomed answer, not as an all-of-UCLA one.
            for(let i=0;i<chain.length;i++){
              setStatus('Ranking results for \u201c'+esc(best.alt)+'\u201d\u2026');
              lastQuery=buildCQL(relaxed,field,chain[i],sortMode);
              const full=await fetchPool(lastQuery,signal,mine);
              if(full===null) return;
              usedScope=chain[i];
              page=full;
              if(full.total){ widenReason=i>0?'empty':''; break; }
            }
          }
        }
        if(!page) return;
      ...
      const bits=[];
      if(relaxedTo) bits.push('No exact match for \u201c'+esc(rankQuery)+'\u201d \u2014 closest matches for \u201c'+esc(relaxedTo)+'\u201d.');
      if(widenReason==='empty') bits.push('Nothing in '+esc(SCOPE[scope].short)+' \u2014 widened to '+esc(SCOPE[usedScope].label)+'.');
      if(widenReason==='weak')  bits.push('Nothing in '+esc(SCOPE[scope].short)+' matched the title \u2014 widened to '+esc(SCOPE[usedScope].label)+'.');
  ...
  function setScope(s,rerun){
    scope=s;
    root.querySelectorAll('.cat-scope .pill').forEach(o=>{
      const on=o.dataset.scope===s; o.classList.toggle('active',on); o.setAttribute('aria-pressed',String(on));
    });
    if(rerun && input.value.trim()) run(true);
  }
  ...
  root.querySelectorAll('.cat-scope .pill').forEach(p=>{
    p.addEventListener('click',e=>{ e.stopPropagation(); setScope(p.dataset.scope,true); });
  });
  $('catGo').addEventListener('click',e=>{ e.stopPropagation(); run(true); });
  $('catClear').addEventListener('click',e=>{
    e.stopPropagation();
    input.value=''; loaded=[]; total=0; parsed=null; ctx=null; out.innerHTML='';
