...
(function(){
  ...
  /* LibCal returns `not-set` for a day nobody has filled in. Rendering those as "unknown" pads
     the list with rows that answer nothing, so they are dropped for that day. A location with
     no hours all week therefore never appears at all. */
  const known=d=>!!d && ((d.times||{}).status||'').toLowerCase()!=='not-set';

  /* Status is a dot *and* a word: colour alone must never carry the meaning. "Open now" is
  ...
  function renderDay(){
    ...
    out.innerHTML=parents.map(p=>{
      let h='<div class="hr-lib">'+row(p,pick,isToday);
